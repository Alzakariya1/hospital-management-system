...
router.get('/saas/overview', verifyToken, requirePermission('hospital.manage'), asyncHandler(async (_req, res) => {
  ...
  const tenantRows = await Promise.all(hospitals.map(async (hospital) => {
    ...
    return {
      ...
      tenant_db_status: hospital.tenant_db_status || 'shared',
      tenant_provisioned_at: hospital.tenant_provisioned_at || hospital.tenant_db_created_at || null,
      tenant_storage_mode: hospital.tenant_db_name ? 'database-per-tenant' : 'shared-database',
