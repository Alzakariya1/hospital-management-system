...
router.delete('/users/:id', verifyToken, requirePermission('admin.users.manage'), asyncHandler(async (req, res) => { await User.deleteOne({ id: Number(req.params.id) }); await audit(req.user.id, `Deleted user ${req.params.id}`, 'users'); res.json({ message: 'User deleted' }); }));
