...
router.put('/doctors/:id', allowRoles('super_admin', 'admin'), asyncHandler(async (req, res) => { const allowed = ['full_name', 'email', 'phone', 'specialization', 'qualification', 'experience_years', 'consultation_fee', 'department_id', 'opd_timing', 'ipd_timing', 'status']; const update = {}; allowed.forEach(k => { if (k in req.body) update[k] = req.body[k] }); if (!Object.keys(update).length) return res.status(400).json({ message: 'No valid fields' }); await Doctor.updateOne({ id: Number(req.params.id) }, { $set: update }); res.json({ message: 'Doctor updated' }); }));
