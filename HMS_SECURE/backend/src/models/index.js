...
const opts = { timestamps: { createdAt: 'created_at', updatedAt: 'updated_at' }, strict: false, versionKey: false };
const counterSchema = new mongoose.Schema({ _id: String, seq: { type: Number, default: 0 } }, { versionKey: false });
const Counter = mongoose.model('Counter', counterSchema);
async function nextId(name) { const c = await Counter.findByIdAndUpdate(name, { $inc: { seq: 1 } }, { upsert: true, new: true }); return c.seq; }
function makeModel(name, collection, extra = {}) { const schema = new mongoose.Schema({ id: { type: Number }, ...extra }, opts); schema.pre('validate', async function (next) { if (this.isNew && !this.id) this.id = await nextId(collection); next(); }); schema.set('toJSON', { transform: (_, ret) => { delete ret._id; return ret; } }); return mongoose.model(name, schema, collection); }
const User = makeModel('User', 'users', { email: { type: String, unique: true, index: true }, status: { type: String, default: 'active' } });
